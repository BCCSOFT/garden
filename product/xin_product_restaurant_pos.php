<?php include("../asset/head/_all/xin_all_front.php"); ?>
<?php include("../asset/head/product/xin_head_product_restaurant_pos.php"); ?>
<?php include("../asset/head/_all/xin_all_link.php"); ?>
<link rel="stylesheet" type="text/css" href="../asset/style/page-css/case.css">
<link rel="stylesheet" type="text/css" href="../asset/style/page-css/feature.css">
<link rel="stylesheet" type="text/css" href="../asset/style/page-css/jabezpos.css">
<link rel="stylesheet" type="text/css" href="../asset/style/page-css/restaurant-solution.css">
<link rel="stylesheet" type="text/css" href="../asset/style/page-css/pos.css">
<?php include("../asset/head/_all/xin_all_script.php"); ?>


 <body id="restaurant-solution_counter">

    <!-- HEADER -->
	<?php include("../asset/include/xin_header.php"); ?>
    <!-- MWNU -->
	<?php include("../asset/include/xin_menu.php"); ?>
    <!-- SUB-MWNU -->
	<?php include("xin_nav_id=restaurant-solution.php"); ?>

    <main>

      <!-- ARTICLE -->
      <article>

        <section class="counter_banner">
          <article>
            <hgroup>
              <h1>櫃檯收銀機</h1>
              <span><img alt="" src="../asset/image/img.png"></span>
              <h5>輕鬆的經營，從高效的收銀開始</h5>
            </hgroup>
            <ol>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <h6>安全穩定</h6>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <h6>彈性擴充</h6>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <h6>直覺操作</h6>
                </figure>
              </li>
            </ol>
            <nav class="product-nav-img">
              <article>
                <ol>
                  <li>
                    <span><img alt="" src="../asset/image/img.png"></span>
                    <a href="xin_product_restaurant_pos_t1.php" data-type="button"></a>
                  </li>
                  <li>
                    <span><img alt="" src="../asset/image/img.png"></span>
                    <a href="xin_product_restaurant_pos_t80-cloud.php" data-type="button"></a>
                  </li>
                  <li>
                    <span><img alt="" src="../asset/image/img.png"></span>
                    <a href="xin_product_restaurant_pos_t80.php" data-type="button"></a>
                  </li>
                </ol>
              </article>
            </nav>
          </article>
        </section>

        <nav class="product-nav counter-nav">
		  <article>
			<ol>
			  <li>
				<a href="xin_product_restaurant_pos_t1.php" data-type="button">雲端基本版</a>
			  </li>
			  <li>
				<a href="xin_product_restaurant_pos_t80-cloud.php" data-type="button">雲端專業版</a>
			  </li>
			  <li>
				<a href="xin_product_restaurant_pos_t80.php" data-type="button">單機專業版</a>
			  </li>
			</ol>
		  </article>
		</nav>

      <?php include("xin_product_pos_id=overview_banner.php"); ?>

        <section class="counter_overview_situation">
          <article>
            <hgroup>
              <h2>IT的問題我們幫你解決</h2>
            </hgroup>
            <ol>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <p>每天對帳好花時間<br> 對帳總金額總是不對？
                  </p>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <p>手寫菜單常常看不懂<br> 客戶點餐大排長龍
                  </p>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <p>找零總是出錯
                  </p>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <p>廚房廚師亂成一團<br> 客戶總是嫌棄上菜慢
                  </p>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <p>奶茶去冰，漢堡不要菜？<br> 顧客喜好好難記？
                  </p>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <p>商品資料建立麻煩<br> 新人總是學不會
                  </p>
                </figure>
              </li>
            </ol>
          </article>
        </section>

         <section class="mobile-pos_overview_pay">
            <article>
              <figure>
                <hgroup>
                  <h2>多元支付收款好方便</h2>
                  <p>支援信用卡、Apple Pay、<br class="mobile">LINE Pay、支付寶...等<br class="desktop"> 多元收款方式，
                    <br> 不論是掃描、感應、晶片、插卡付款都可以。
                  </p>
                  <span><img alt="" src="../asset/image/img.png"></span>
                </hgroup>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                </figure>
              </figure>
            </article>
          </section>

          <section class="mobile-pos_overview_e-invoice">
            <article>
              <figure>
                <hgroup>
                  <h2>電子發票辦到好</h2>
                  <p>您知道政府即將停止使用電子計算機發票嗎？<br> 為免造成營運成本增加與罰鍰問題，
                    <br> 儘早換用電子發票好處多多，
                    <br> 不僅可省錢、減稅，還能簡化報稅流程。
                  </p>
                <a href="../feature/xin_feature_invoice.php" target="_blank" data-type="button">了解更多 </a>
                </hgroup>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                </figure>
              </figure>
            </article>
          </section>

          <section class="mobile-pos_overview_e-feature">
            <article>
              <hgroup>
                <h2>電子發票功能</h2>
              </hgroup>
              <ol>
                <li>
                  <figure>
                    <span><img alt="" src="../asset/image/img.png"></span>
                    <h5>發票取配號</h5>
                  <p>可執行電子發票開立、<br>作廢、退貨、折讓功能
                  </p>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <h5>共通載具碼登錄</h5>
                  <p>提供使用共通載具<br>索取電子發票
                  </p>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <h5>愛心碼捐贈發票</h5>
                  <p>提供輸入社福團體愛心碼<br>捐贈電子發票
                  </p>
                </figure>
              </li>
              <li>
                <figure>
                  <span><img alt="" src="../asset/image/img.png"></span>
                  <h5>安全水位控管</h5>
                  <p>有效控管門市/收銀機<br>電子發票號碼之索取配號<br>及庫存安全水位
                  </p>
                </figure>
              </li>
            </ol>
          </article>
        </section>

        <section class="counter_overview_bpos">
          <article>
            <figure>
              <hgroup>
                <h2>雲端基本版</h2>
                <p>適合先結餐廳。<br>如：飲料店、小吃店、冰店、美食街等
                </p>
                <ol>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>跨平台</h6>
                    </figure>
                  </li>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>雲端更新</h6>
                    </figure>
                  </li>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>月租收費</h6>
                    </figure>
                  </li>
                </ol>
                <a href="xin_product_restaurant_pos_t1.php" data-type="button">了解更多 </a>
              </hgroup>
              <figure>
                <span><img alt="" src="../asset/image/img.png"></span>
              </figure>
            </figure>
          </article>
        </section>

        <section class="counter_overview_b1pos-cloud">
          <article>
            <figure>
              <hgroup>
                <h2>雲端專業版</h2>
                <p>適合後結餐廳。<br>如：西餐廳、具附餐、吃到飽餐廳等
                </p>
                <ol>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>windows</h6>
                    </figure>
                  </li>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>雲端更新</h6>
                    </figure>
                  </li>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>桌位管理</h6>
                    </figure>
                  </li>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>月租收費</h6>
                    </figure>
                  </li>
                </ol>
                <a href="xin_product_restaurant_pos_t80-cloud.php" data-type="button">了解更多 </a>
              </hgroup>
              <figure>
                <span><img alt="" src="../asset/image/img.png"></span>
              </figure>
            </figure>
          </article>
        </section>

        <section class="counter_overview_b1pos">
          <article>
            <figure>
              <hgroup>
                <h2>單機專業版</h2>
                <p>適合後結餐廳。<br>如：西餐廳、具附餐、吃到飽餐廳等
                </p>
                <ol>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>windows</h6>
                    </figure>
                  </li>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>本機備份</h6>
                    </figure>
                  </li>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>桌位管理</h6>
                    </figure>
                  </li>
                  <li>
                    <figure>
                      <span><img alt="" src="../asset/image/img.png"></span>
                      <h6>一次買斷</h6>
                    </figure>
                  </li>
                </ol>
                <a href="xin_product_restaurant_pos_t80.php" data-type="button">了解更多 </a>
              </hgroup>
              <figure>
                <span><img alt="" src="../asset/image/img.png"></span>
              </figure>
            </figure>
          </article>
        </section>

        <!-- 適用餐飲類別 -->
        <?php include("xin_product_id=restaurant_type.php"); ?>
        <!-- 成功案例 -->
        <?php include("../case/xin_case_id=restaurant-picitem.php"); ?>
        <!-- 智能餐飲流程圖 -->
        <?php include("../solution/xin_solution_id=restaurant_flow.php"); ?>

        <!-- 功能模組 -->
        <?php include("../feature/xin_feature_id=reataurant-list.php"); ?>
        <?php include("../feature/xin_feature_id=reataurant-purchase-list.php"); ?>

        <!-- 硬體推薦 -->
        <?php include("../hardware/xin_hardware_id=restaurant.php"); ?>

        <!-- 後台管理 -->
        <?php include("../software/xin_software_jabezpos_id=restaurant_fearure.php"); ?>

        <!-- 影音介紹 -->
        <section class="mobile-pos_overview_video-intro" data-light-box>
          <ol>
            <li>
              <figure>
                <h2>櫃檯收銀機</h2>
                <h4>概觀篇</h4>
                <span><img alt="" src="../asset/image/img.png"></span>
                <a href="#" data-type="button"></a>
              </figure>
            </li>
            <li>
              <figure>
                <h2>櫃檯收銀機</h2>
                <h4>操作篇</h4>
                <span><img alt="" src="../asset/image/img.png"></span>
                <a href="#" data-type="button"></a>
              </figure>
            </li>
            <li>
              <figure>
                <h2>櫃檯收銀機</h2>
                <h4>應用篇</h4>
                <span><img alt="" src="../asset/image/img.png"></span>
                <a href="#" data-type="button"></a>
              </figure>
            </li>
          </ol>
        </section>

        <!-- 售後服務 -->
        <?php include("xin_product_id=service.php"); ?>
        <!-- 型錄下載 -->
        <?php include("xin_product_id=download.php"); ?>

      </article>
      <!-- ASIDE -->
      <aside></aside>

    </main>
     <!-- FOOTER -->
	<?php include("../asset/include/xin_footer.php"); ?>

</body>
<?php include("../asset/include/xin_foot.php"); ?>